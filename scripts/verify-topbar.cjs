 ...
 *   - items with no page in this app show an explicit "Coming soon" state
 *     (no silent no-ops)
 *
 ...
 *           link (if present)
 *       a3) every apps-menu item either navigates to a real route or renders
 *           an explicit "Coming soon" state (no silent no-op)
 *
 *   PART B — live checks
 ...
 *
 * Requires scripts/sql/13_notifications_table.sql (table + RLS + triggers).
 *
 ...
 * auth accounts. Usage: node scripts/verify-topbar.cjs
 */
...
function partA() {
  ...
  const appsBlock = TOPBAR_SRC.slice(TOPBAR_SRC.indexOf('TOPBAR_APPS'));
  const a3 =
    appsBlock.includes("to: '/dashboard'") &&
    appsBlock.includes("to: '/projects'") &&
    appsBlock.includes("to: '/services'") &&
    appsBlock.includes("to: '/billing'") &&
    appsBlock.includes('comingSoon: true') &&
    TOPBAR_SRC.includes('Coming soon') &&
    !TOPBAR_SRC.includes('topbarApps');
  record(
    'a3) apps items navigate to real routes or show explicit "Coming soon"',
    a3,
    a3 ? '' : 'apps menu still has silent no-ops or static misc data',
  );
}
